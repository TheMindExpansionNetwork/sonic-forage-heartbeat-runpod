...
Usage::
    ...
    uv run python scripts/collect_decoder_calibration.py --num-prompts 8 --max-calls 16
...
# Seq length (latent frames) for the calibration. Matches the 60s
# profile that the XL refit_b4_60s engine was built for.
SEQ_LEN = 1500


def _resolve_xl_engines() -> dict[str, str]:
    """Find the XL TRT engines needed to drive calibration."""
...
def main():
    ...
    print("[setup] Locating XL TRT engines...")
    engines = _resolve_xl_engines()
    for k, v in engines.items():
        print(f"  {k}: {v}")

    print("[setup] Loading XL session...")
    session = Session(
