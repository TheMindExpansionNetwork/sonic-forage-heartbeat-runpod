...
interface LoraState {
   ...
   *  lowered the ceiling, or a source swap landed on a tighter tier).
   *  Drops the LATEST-added entries first (Set insertion order). */
...
export const useLoraStore = create<LoraState>((set, get) => ({
  ...
  reset: () =>
    set((s) => {
      ...
      _refitTimers.clear();
      // Clip the seed against the active cap so reset can't restore
      // more LoRAs than the current tier allows.
      return {
        catalog,
        strengths,
