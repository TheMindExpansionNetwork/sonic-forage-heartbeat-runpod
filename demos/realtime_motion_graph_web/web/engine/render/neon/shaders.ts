// GLSL shader source for the neon visualizer. Verbatim copy of the
// shaders/*.glsl files in wayne-wu/webgl-music-visualizer, inlined as TS
// strings so we don't need a webpack/turbopack GLSL loader.
...
float ease_in_quadratic(float t) { return t * t; }
...
vec3 cubic(vec3 t)   { return t * t * (3.0 - 2.0 * t); }
vec3 quintic(vec3 t) { return t * t * t * (t * (t * 6.0 - 15.0) + 10.0); }

float grad(vec3 i, vec3 f, vec3 inc) { return dot(hash3(i + inc), f - inc); }
...
float gain(float g, float t) {
  if (t < 0.5) return bias(g, 2.0 * t) / 2.0;
  else return bias(1.0 - g, 2.0 * t - 1.0) / 2.0 + 0.5;
}
