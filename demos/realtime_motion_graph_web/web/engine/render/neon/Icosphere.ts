// Adapted from wayne-wu/webgl-music-visualizer (src/geometry/Icosphere.ts).

import Drawable from "./Drawable";
import { gl } from "./globals";
import { vec3, vec4, type Vec3 } from "./mat";
...
class Icosphere extends Drawable {
  ...
  constructor(center: Vec3, radius: number, subdivisions: number, mode: GLenum) {
    super();
    this.radius = radius;
  ...
  create() {
    ...
    const vertices: Array<Float32Array> = new Array(12);
    for (let i = 0; i < 12; ++i) {
    ...
    for (let s = 0; s < this.subdivisions; ++s) {
      ...
      function mid(v0: number, v1: number): number {
        ...
        if (idx === undefined) {
          const midpoint = new Float32Array(
            buffer0,
            vertexByteOffset + vertices.length * 4 * Float32Array.BYTES_PER_ELEMENT,
            4,
          );
      ...
      for (let t = 0; t < triangles.length; ++t) {
        const v0 = triangles[t][0];
        const v1 = triangles[t][1];
        const v2 = triangles[t][2];
        const v3 = mid(v0, v1);
        const v4 = mid(v1, v2);
        const v5 = mid(v2, v0);

        const t0 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3,
        ));
        const t1 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3,
        ));
        const t2 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3,
        ));
        const t3 = (nextTriangles[triangleIdx] = new Uint32Array(
          buffers[b],
          indexByteOffset + triangleIdx++ * 3 * Uint32Array.BYTES_PER_ELEMENT,
          3,
        ));
